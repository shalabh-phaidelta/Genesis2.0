...
# @app.get('/metrics/warehouse/{warehouse_id}', summary = 'Get details of current user warehouse')
# async def get_metric_warehouse( warehouse_id : int , user : SystemUser = Depends(get_current_user)):
...
# @app.get('/metrics/warehouse/{warehouse_id}/unit/{unit_id}', summary = 'Get details of current user for unit')
# async def get_metric_unit(warehouse_id : int , unit_id : int, user : SystemUser = Depends(get_current_user)):
...
async def login(from_data : OAuth2PasswordRequestForm = Depends()):
    ...
    if not verify_password(from_data.password, hashed_password):
        ...
        # need to insert access_token after every login
